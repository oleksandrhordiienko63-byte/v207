	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (!hasMoreToolCalls && !hasProducedEdit && emptyTurnRetries >= EMPTY_TURN_MAX && pathsAlreadyRead.size > 0) {
				...
				pendingMessages.push({
					role: "user",
					content: [{ type: "text", text: `You are about to finish with ZERO edits. This guarantees a loss. You read \`${topFile}\`. Apply \`edit\` to it now — even a partial or imperfect change scores more than nothing.` }],
					timestamp: Date.now(),
				});
				hasMoreToolCalls = false;
				continue;
			...
			if (hasMoreToolCalls) {
				...
				for (let i = 0; i < toolResults.length; i++) {
					...
					if (tr.isError) {
						...
						// Auto-fix: specific edit error recovery
						...
						try {
							...
							if (lsResult.status === 0 && lsResult.stdout) {
								const siblings = lsResult.stdout.trim().split("\n")
									.map((f: string) => dir === "." ? f : dir + "/" + f)
									.filter((f: string) => !editedPaths.has(f) && !editedPaths.has(f.replace(/^\.\//, "")));
								const related = siblings.filter((f: string) => {
									const name = f.split("/").pop() || "";
									return name.includes(".test.") || name.includes(".spec.") || name.includes("_test.")
										|| name.includes(".freezed.") || name.includes(".g.") || name.includes(".generated.")
										|| (ext && f.endsWith(ext));
								}).slice(0, 5);
								if (related.length > 0) {
									for (const rf of related) { if (!foundFiles.includes(rf)) foundFiles.push(rf); }
				...
				for (const tr of toolResults) {
					...
					// Catch fd/rg tool crashes and suggest bash fallback
				...
				if (
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: `Discovery stall: ${totalExplorationSteps} read/bash steps with no \`edit\` yet. The top-ranked target is \`${primary}\` — \`read\` it if needed, then \`edit\` it immediately. Do not run more broad directory listing.`,
							},
						],
						timestamp: Date.now(),
					});
				...
				if (!hasProducedEdit && pendingMessages.length === 0 && (Date.now() - loopStart) >= 45_000 && pathsAlreadyRead.size > 0) {
					...
					if (topFile) {
						pendingMessages.push({
							...
							content: [{
								type: "text",
								text: `CRITICAL: ${Math.round((Date.now() - loopStart)/1000)}s elapsed with ZERO edits. An empty diff scores zero — you LOSE. You have read \`${topFile}\`. Call \`edit\` on it NOW with your best guess. Any edit beats zero. Do NOT search more. EDIT NOW.`,
							}],
							timestamp: Date.now(),
						});
					}
